...
import require$$1, { randomUUID } from "crypto";
import net from "net";
...
const Event = z$1.object({
  name: z$1.string(),
  handler: z$1.function()
});
const InitHook = z$1.function();
const Hooks = z$1.object({
  init: InitHook.optional()
});
const ContentTypeAction = z$1.enum(["create", "update", "delete"]);
const PopulateConfig = z$1.union([
  z$1.literal("*"),
  z$1.literal(true),
  z$1.array(z$1.string()),
  z$1.record(z$1.any())
]);
const ContentType = z$1.object({
  uid: z$1.string(),
  actions: z$1.array(ContentTypeAction).optional(),
  populate: PopulateConfig.optional()
});
const Socket = z$1.object({ serverOptions: z$1.unknown().optional() });
const plugin = z$1.object({
  events: z$1.array(Event).optional(),
  hooks: Hooks.optional(),
  contentTypes: z$1.array(z$1.union([z$1.string(), ContentType])),
   ...
   */
  sensitiveFields: z$1.array(z$1.string()).optional()
});
const config = {
  default() {
    return {
...
z.record(z.string(), z.any()).describe("Apply filters to the query");
z.string().describe("Specify the locale for localized content");
z.enum([
  "draft",
  "published"
]).describe("Filter by publication status");
z.string().describe("Search query string");
...
const settingsSchema = z$2.object({
  enabled: z$2.boolean().optional(),
  cors: z$2.object({
    origins: z$2.array(
      z$2.string().refine(isValidCorsOrigin, {
        message: 'Each origin must be a valid http(s) URL or "*"'
      })
    ).optional()
  }).passthrough().optional(),
  connection: z$2.object({
    maxConnections: z$2.number().int().positive().max(1e5).optional(),
    pingTimeout: z$2.number().int().positive().max(6e5).optional(),
    pingInterval: z$2.number().int().positive().max(6e5).optional(),
    connectionTimeout: z$2.number().int().positive().max(6e5).optional()
  }).optional(),
  security: z$2.object({
    requireAuthentication: z$2.boolean().optional(),
    rateLimiting: z$2.object({
      enabled: z$2.boolean().optional(),
      maxEventsPerSecond: z$2.number().int().positive().max(1e5).optional()
    }).optional(),
    ipWhitelist: z$2.array(z$2.string().refine(isValidIpOrCidr, {
      message: "Each entry must be a valid IPv4/IPv6 address or CIDR"
    })).max(1e3).optional(),
    ipBlacklist: z$2.array(z$2.string().refine(isValidIpOrCidr, {
      message: "Each entry must be a valid IPv4/IPv6 address or CIDR"
    })).max(1e3).optional()
  }).optional(),
  events: z$2.object({
    customEventNames: z$2.boolean().optional(),
    includeRelations: z$2.boolean().optional(),
    excludeFields: z$2.array(z$2.string()).optional(),
    onlyPublished: z$2.boolean().optional()
  }).optional(),
  rooms: z$2.object({}).passthrough().optional(),
  redis: z$2.object({
    enabled: z$2.boolean().optional(),
    url: z$2.string().optional()
  }).optional(),
  namespaces: z$2.object({}).passthrough().optional(),
  middleware: z$2.object({}).passthrough().optional(),
  monitoring: z$2.object({
    enableConnectionLogging: z$2.boolean().optional(),
    enableEventLogging: z$2.boolean().optional(),
    maxEventLogSize: z$2.number().int().positive().optional()
  }).optional(),
  entitySubscriptions: z$2.object({}).passthrough().optional(),
  presence: z$2.object({}).passthrough().optional(),
  livePreview: z$2.object({}).passthrough().optional(),
  fieldLevelChanges: z$2.object({}).passthrough().optional(),
  rolePermissions: z$2.record(z$2.any()).optional()
}).passthrough();
